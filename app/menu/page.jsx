...
import { ShoppingCart, X, LayoutGrid, Clock } from "lucide-react";
...
export default function PremiumPOSPage() {
  ...
  const { data: foodRes, isLoading } = useQuery({
    ...
    queryFn: () => FoodService.getAllFoods(`search=${search}&category=${category}&page=${page}`),
  });
  ...
  return (
    <div className="min-h-screen w-full  flex bg-[#080808] text-zinc-100 font-sans selection:bg-indigo-500/30 overflow-hidden">
      ...
      <aside className="hidden xl:flex w-[25%] h-screen bg-[#080808] flex-col relative">
        ...
        <div className="flex-1 overflow-y-auto p-6 no-scrollbar relative z-10">
          <div className="flex items-center justify-between mb-8">
            <h2 className="text-xl font-bold tracking-tight italic">Current Order</h2>
      ...
      {/* MOBILE DRAWER (Logic remains same, styling updated) */}
      <AnimatePresence>
        {drawerOpen && (
          <>
            <motion.div 
            ...
            >
              <div className="p-6 overflow-y-auto no-scrollbar">
                <div className="flex justify-between items-center mb-8">
                   ...
                   <button onClick={() => setDrawerOpen(false)} className="p-3 bg-white/5 rounded-full hover:bg-white/10 transition-colors"><X size={20}/></button>
                </div>
                <CheckoutPanel cart={cart} subtotal={subtotal} tax={subtotal * 0.125} total={subtotal * 1.125} setCart={setCart} clearCart={() => setCart([])} onOrderSuccess={() => setDrawerOpen(false)} />
              </div>
            </motion.div>
          </>
        )}
