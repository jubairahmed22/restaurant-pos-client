...
import { getShopProduct, updateShopProduct, getShopCategories } from '@/services/shop.service';
...
export default function EditShopProductPage() {
  ...
  const { data } = useQuery({ queryKey: ['shop-product', id], queryFn: () => getShopProduct(id), staleTime: 0 });
  ...
  useEffect(() => {
    if (data?.data && !loaded) {
      ...
      setForm({
        ...
        description: p.description || '', category: p.category?._id || '',
        brand: p.brand || '', sku: p.sku || '',
        price: String(p.price || ''), comparePrice: String(p.comparePrice || ''),
        stock: String(p.stock ?? 0),
        discountType: p.discountType || 'none', discountValue: String(p.discountValue || 0),
        discountEndDate: p.discountEndDate ? p.discountEndDate.substring(0, 10) : '',
        ...
        attributes: p.attributes?.length
          ? p.attributes.map((a: Attribute) => ({
              name: a.name,
              options: a.options?.length ? a.options.map((o: AttributeOption) => ({ label: o.label, value: o.value, unit: o.unit || '' })) : [EMPTY_OPTION()],
            }))
          : [],
        tags: p.tags?.join(', ') || '',
  ...
  const mutation = useMutation({
    mutationFn: () => updateShopProduct(id, {
    ...
    onSuccess: () => { toast.success('Product updated'); router.push('/dashboard/shop/products'); },
    onError: (err: unknown) => {
    ...
    <div className="space-y-5 max-w-5xl">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <h1 className="text-2xl font-extrabold text-[#1B3A6B] leading-none">Edit Product</h1>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1fr_320px] gap-5">
        ...
        <div className="space-y-5">
          ...
          <div className={card}>
            ...
            <div><label className={label}>Title *</label><input className={inp} value={form.title} onChange={e => set('title', e.target.value)} /></div>
            <div><label className={label}>Short Description</label><input className={inp} value={form.shortDescription} onChange={e => set('shortDescription', e.target.value)} /></div>
            <div><label className={label}>Full Description</label><textarea className={inp} rows={5} value={form.description} onChange={e => set('description', e.target.value)} /></div>
          </div>
          ...
          <div className={card}>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              <div><label className={label}>Price (AUD) *</label><input type="number" className={inp} value={form.price} onChange={e => set('price', e.target.value)} min="0" step="0.01" /></div>
              <div><label className={label}>Compare Price</label><input type="number" className={inp} value={form.comparePrice} onChange={e => set('comparePrice', e.target.value)} min="0" step="0.01" /></div>
              ...
              <div><label className={label}>SKU</label><input className={inp} value={form.sku} onChange={e => set('sku', e.target.value)} /></div>
            </div>
          </div>
          ...
          <div className={card}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              ...
              {form.discountType !== 'none' && (
                <>
                  <div><label className={label}>Value</label><input type="number" className={inp} value={form.discountValue} onChange={e => set('discountValue', e.target.value)} min="0" /></div>
          ...
          <div className={card}>
            ...
            {form.attributes.length === 0 && <p className="text-xs text-slate-400 italic">No attributes yet.</p>}
            ...
            <div className="space-y-2">
              {form.specifications.map((spec, i) => (
                <div key={i} className="flex gap-2">
                  <input className={`${inp} flex-1`} value={spec.key} onChange={e => updateSpec(i, 'key', e.target.value)} placeholder="Key" />
                  <input className={`${inp} flex-1`} value={spec.value} onChange={e => updateSpec(i, 'value', e.target.value)} placeholder="Value" />
                  {form.specifications.length > 1 && <button onClick={() => removeSpec(i)} className="p-2 text-red-400 hover:bg-red-50 rounded-lg"><Trash2 size={14} /></button>}
                </div>
              ))}
            </div>
            <button onClick={addSpec} className="flex items-center gap-2 text-sm text-[#1B3A6B] hover:underline font-medium"><Plus size={14} /> Add Spec</button>
          </div>

          {/* SEO */}
          <div className={card}>
            <h2 className="font-bold text-slate-700 text-sm uppercase tracking-wider">SEO</h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        ...
        <div className="space-y-5">
          <div className={card}>
            ...
            <div><label className={label}>Brand</label><input className={inp} value={form.brand} onChange={e => set('brand', e.target.value)} /></div>
            <div><label className={label}>Tags</label><input className={inp} value={form.tags} onChange={e => set('tags', e.target.value)} placeholder="tag1, tag2" /></div>
            <div><label className={label}>Weight</label><input className={inp} value={form.weight} onChange={e => set('weight', e.target.value)} /></div>
            <div><label className={label}>Dimensions</label><input className={inp} value={form.dimensions} onChange={e => set('dimensions', e.target.value)} /></div>
          </div>
