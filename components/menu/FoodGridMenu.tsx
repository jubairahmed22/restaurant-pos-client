...
export default function FoodGridMenu({ foods, categories, search, category, updateURL, onAdd, isLoading }: any) {
  ...
  const groupedFoods = useMemo(() => {
    ...
    return categories
      .map((cat: any) => ({
        ...cat,
        foods: foods.filter((food: any) => food.category === cat._id || food.category?._id === cat._id),
      }))
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex items-center gap-2 overflow-x-auto no-scrollbar scroll-smooth">
            <ListButtonWhite
              ...
              className="px-4 py-1.5 text-xs" // Assuming ListButtonWhite accepts className
            />
      ...
      <div className="space-y-12 mt-4">
        {isLoading ? (
          ...
          groupedFoods.map((group: any) => (
            <section key={group._id} className="animate-in fade-in slide-in-from-bottom-2 duration-500">
              <div className="flex items-center gap-3 mb-6">
                <h2 className="text-xl font-serif italic text-white/80">{group.title}</h2>
        ...
        {!isLoading && groupedFoods.length === 0 && (
          <div className="flex flex-col items-center justify-center py-24 opacity-60">
            <p className="text-zinc-400 text-sm font-light italic">No culinary matches found.</p>
          </div>
        )}
      </div>
    </div>
